...
impl eframe::App for STM32EguiMonitor {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ctx, |cui| {
            ...
            egui::Window::new("probe interface test").show(ctx, |cui| {
                cui.heading("probe interface test");
                cui.horizontal(|cui| {
                    cui.label("waching");

                    if cui.button("start").clicked() {
                        self.my_probe
                            .watching_start(std::time::Duration::from_millis(100));
                    }

                    if cui.button("stop").clicked() {
                        self.my_probe.watching_stop();
                    }
                });

                cui.label(format!("data: {}", self.my_probe.get_data()));
            });
