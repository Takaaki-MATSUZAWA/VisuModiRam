...
impl DataViewerTab {
                ...
                egui::ScrollArea::vertical()
                    ...
                    .show(ui, |ui| {
                        for var_name in &data_file.variables {
                            let mut is_visible = self.plot_settings.visible_variables.contains(var_name);
                            if ui.checkbox(&mut is_visible, var_name).changed() {
                                if is_visible {
                                    ...
                                    self.plot_settings.visible_variables.retain(|x| x != var_name);
                                }
                            }
                        }
                    });
    ...
    fn data_table_ui(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ui.label("Show last:");
            ui.add(egui::DragValue::new(&mut self.table_rows_to_show).range(10..=10000).suffix(" rows"));
        });
        ...
        let visible_vars: Vec<String> = self.plot_settings.visible_variables
            .iter()
        ...
        // 最大行数を決定
        let max_rows = visible_vars.iter()
                ...
                if let Some(data) = filtered_data.get(var_name) {
                    if !data.is_empty() {
                        ...
                        let colors = [
                            ...
                            Color32::LIGHT_BLUE, Color32::LIGHT_GREEN, Color32::LIGHT_RED,
                        ];
                        ...
                        if self.plot_settings.show_points {
                            let points_for_display: PlotPoints = data.iter()
                                .map(|[time, value]| [*time, *value])
                                .collect();
                            plot_ui.points(
            ...
            for var_name in &self.plot_settings.visible_variables {
                if let Some(data) = filtered_data.get(var_name) {
                    if !data.is_empty() {
                        ...
                        ui.label(format!("{}: Min={:.3}, Max={:.3}, Avg={:.3}, StdDev={:.3}, Samples={}", 
                            var_name, min_val, max_val, avg_val, std_dev, data.len()));
                    }
                }
            }
        });
    ...
    fn get_filtered_data(&self) -> HashMap<String, Vec<[f64; 2]>> {
        ...
        for var_name in &self.plot_settings.visible_variables {
            if let Some(data) = self.loaded_data.get(var_name) {
                let filtered_data: Vec<[f64; 2]> = data.iter()
                    .filter(|[time, _]| {
                        let after_start = self.plot_settings.time_range_start
                            .map_or(true, |start| *time >= start);
                        let before_end = self.plot_settings.time_range_end
    ...
    fn read_csv_data(&mut self, path: &PathBuf) -> Result<HashMap<String, Vec<[f64; 2]>>, String> {
        ...
        for line_result in lines {
            ...
            // タイムスタンプ解析
            let timestamp = values[0].trim().parse::<f64>()
