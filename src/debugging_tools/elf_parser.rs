...
impl ELFParser {
    ...
    #[cfg(test)]
    pub fn test_chip_detection(elf_path: &str) -> Option<String> {
        let parser = ELFParser {
            elf_path: elf_path.to_string(),
            variable_list: Arc::new(Mutex::new(Vec::new())),
            scan_prgress: Arc::new(Mutex::new(0.0)),
        };
        parser.guess_chip_name()
    }
    ...
    fn guess_from_linker_script(&self, project_root: &std::path::Path) -> Option<String> {
        ...
        if let Ok(entries) = fs::read_dir(project_root) {
            ...
            debug!("Could not read project directory: {:?}", project_root);
        }
        None
    }
    ...
    fn guess_from_project_name(&self, project_root: &std::path::Path) -> Option<String> {
        ...
        if let Some(project_name) = project_root.file_name()?.to_str() {
            ...
            if project_name.contains("G474") {
                ...
                return Some("STM32G474RETx".to_string());
            }
            if project_name.contains("F103") {
                debug!("Found F103 in project name");
                return Some("STM32F103RETx".to_string());
            }
            if project_name.contains("F401") {
                debug!("Found F401 in project name");
                return Some("STM32F401RETx".to_string());
            }
            if project_name.contains("F411") {
                debug!("Found F411 in project name");
                return Some("STM32F411RETx".to_string());
            }
            if project_name.contains("F446") {
                debug!("Found F446 in project name");
                return Some("STM32F446RETx".to_string());
            }
            if project_name.contains("H743") {
    ...
    fn guess_from_cmake_files(&self, project_root: &std::path::Path) -> Option<String> {
        ...
        for file_name in files_to_check {
            ...
            if let Ok(content) = fs::read_to_string(&file_path) {
                // STM32G474xx -> STM32G474RETx のような変換
                if content.contains("STM32G474") {
                    return Some("STM32G474RETx".to_string());
                }
                if content.contains("STM32F103") {
                    return Some("STM32F103RETx".to_string());
                }
                if content.contains("STM32F401") {
                    return Some("STM32F401RETx".to_string());
                }
                if content.contains("STM32F411") {
                    return Some("STM32F411RETx".to_string());
                }
                if content.contains("STM32F446") {
                    return Some("STM32F446RETx".to_string());
                }
                if content.contains("STM32H743") {
                    return Some("STM32H743VITx".to_string());
                }
            }
        }
...
pub fn search_target_mcu_name(elf_file_path: &PathBuf) -> Option<String> {
    ...
    if ioc_file_path.is_file() {
        debug!("Found .ioc file: {:?}", ioc_file_path);
    ...
    // ELFを解析してMCUの名前を特定（startupファイルから）
        ...
        .map(|p| p.join("CMakeFiles/rules.ninja"))
    {
        if ninja_file_path.is_file() {
            debug!("Found rules.ninja file: {:?}", ninja_file_path);
    ...
    debug!("Could not determine chip name from any method");
    None
}
